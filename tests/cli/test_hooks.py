...
def test_install_and_uninstall_pre_commit_by_default(repo: Path) -> None:
    ...
    expected_header = "#!/bin/sh\n# Generated by prec. Do not edit.\n# prec-managed-hook:v1\n"
    assert script.startswith(expected_header)
...
def test_marker_text_outside_header_does_not_claim_hook(repo: Path) -> None:
    hook = repo / ".git/hooks/pre-commit"
    hook.write_text("#!/bin/sh\necho '# prec-managed-hook:v1'\n")
    hook.chmod(0o755)
...
def test_legacy_managed_hook_can_be_upgraded_and_uninstalled(repo: Path) -> None:
    hook = repo / ".git/hooks/pre-commit"
    legacy_header = "#!/bin/sh\n# Generated by prec. Do not edit.\n# prec-managed-hook\n"
    hook.write_text(legacy_header + "exit 0\n")
    hook.chmod(0o755)
